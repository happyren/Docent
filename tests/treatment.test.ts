...
describe("the house treatment (D120)", () => {
  ...
  it("clamps roughness and stroke to the house's own, never up", () => {
    // Architect ink (D143): anything rougher comes all the way down.
    expect(treatElement({ roughness: 2 }).roughness).toBe(0);
    expect(treatElement({ roughness: 1 }).roughness).toBe(0);
  ...
  it("returns the same record when nothing changes", () => {
    const el = { fillStyle: "solid", roughness: 0, strokeWidth: 2, points: [[0, 0]] };
  ...
  it("dresses a whole drawing and leaves the rest of each element alone", () => {
    const drawing = [
      ...
      { fillStyle: "solid", roughness: 0, type: "ellipse" },
    ];
