...
- **Layout engines as dependencies** — Graphviz/WASM, ELK, dagre, Cytoscape are not taken on as runtime dependencies (I7); the layered pipeline is Docent's own (D74). A `layout/1` **plugin contract** (an out-of-process engine given a graph, answering positions) is the intended door for Graphviz or ELK on the desktop, a later decision. Frame-level arrangement of Layer 1 and a BeauVis-style rating harness are likewise **noted, not built** *(noted A18)*
