...
| D23 *(2026-08-18)* | **Bundled shape library as a static asset.** `public/libraries/software-architecture.excalidrawlib` — the "Software Architecture" library by Youri Tjang (`youritjang`), from `excalidraw/excalidraw-libraries` (MIT, per that repository's LICENSE), vendored verbatim and attributed in the README. It is fetched from the app's own origin and merged into the library sidebar at startup by the excalidraw adapter (B1), non-fatally — a missing or malformed asset warns and the canvas comes up regardless | Self-hosting means the shapes are there on first load, with no call out to libraries.excalidraw.com. I7 governs npm runtime weight; a static file ships no code, no transitive tree, and no upgrade surface, so it is recorded here for provenance rather than counted as a dependency — the runtime set stays `react`, `react-dom`, `@excalidraw/excalidraw` (D12). The library API is Excalidraw's, so exactly one module may touch it (B1); vendoring the file rather than fetching upstream at runtime keeps the deployment offline-capable and pins what users get |
