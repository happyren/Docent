...
| S13 | Desktop distribution *(added A4)* | Docent ships as a **Tauri** desktop app for macOS, Windows, and Linux, wrapping the **same built SPA** — no second frontend, no fork (I1). The desktop portfolio is a native store implementing the **same D17 file-tree contract and `/api` route contract** as the self-host store, rooted in the OS app-data directory; contract parity is tested. Samples and bundled libraries ship in the app. The MCP agent endpoint is **not** part of desktop v1 — agent control remains a self-host capability; the desktop serves the watching and reading audiences. Installers build in CI; auto-update is out of scope for v1 |
| S14 | GitHub project sync *(added A5)* | A portfolio project may **bind to a GitHub repository** (`owner/repo`, path prefix, branch, and an API base URL so GitHub Enterprise instances work). Scenes in a bound project read and write **through GitHub's HTTP API** — every save is a commit; loads fetch the current file; deletes commit removals. Writes carry the file's last-known SHA, so a scene changed remotely since it was loaded is a **loud conflict**, never a silent overwrite. Auth is a **fine-grained personal access token** — chosen over OAuth device flow because customer-controlled GitHub instances may not host any OAuth app; tokens are held outside the data tree and are write-only through the API (never echoed). Both store implementations honor the same binding contract; unbound projects behave exactly as before |
