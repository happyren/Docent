...
| D39 *(A8, 2026-08-21)* | **Spotlight holes are per target, never per stroke.** The dim backdrop's `evenodd` holes are cut one per highlighted *target* — a composite or group is one hole over the union of its members — and holes that overlap are merged into their enclosing box before cutting, so the hole set is always disjoint. A lone plain shape keeps its true outline (ellipse, diamond); anything made of several parts gets its box | Overlapping holes under `evenodd` cancel, which is exactly the half-darkened patches inside imported icons: one hole per stroke of a glyph re-dims every overlap. Disjoint holes make the fill rule sound again, and a union box is what a reader sees as "the icon" anyway |
