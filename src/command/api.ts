...
export interface TidyResult {
  ...
  /** The craft score (D76) either side of the tidy. */
  score: { before: number; after: number };
  next: string;
}

/** The craft score (D76) either side of a tidy. */
function craftScores(beforeSnapshot: SceneSnapshot, afterLint: LintReport): TidyResult["score"] {
  return { before: lint(beforeSnapshot).score.score, after: afterLint.score.score };
}
...
export class CommandAPI {
  ...
  async tidy(scope: TidyScope): Promise<TidyResult> {
    ...
    if (!targets.ops.length) {
      const unchanged = lint(before).score.score;
      return { tidied: { frames: 0, components: 0 }, score: { before: unchanged, after: unchanged }, next: "nothing to tidy in that scope — tidy({all:true}) formats the diagram" };
    }
    ...
    this.setWorking(false);
    const score = craftScores(before, lint(after));
    return {
      tidied: { frames: targets.frames, components: targets.components },
      score,
      next: `craft score ${score.before} → ${score.after}; the changelog is empty by construction — validate() for what is left, or save_scene`,
    };
  }
