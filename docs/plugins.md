...
| `run` | How to start the provider: `command` (looked up on your **login shell's PATH** — plus `~/.local/bin`, `~/.cargo/bin`, `/opt/homebrew/bin`, `/usr/local/bin` — so a tool installed for your shell, like `uvx`, is found when Docent is started from the Dock or relaunched by an update), `args` (one of them must contain `{port}`, which Docent replaces with a free loopback port; `DOCENT_PLUGIN_PORT` is set in the environment too), and `health` — a path polled every 500 ms until it answers any HTTP status. There is no deadline while the process lives: a first run may be downloading weights. The process exiting marks the plugin *failed* with its exit status |
