...
- **The craft score.** `validate()` returns a score out of 100 with its parts — crossings, bends, alignment, overlaps, edge lengths, angles, colour distance — weighted as the research ranks them, per frame, with one line of advice each. The test suite watches a fixture's score, so the pipeline cannot regress quietly.
