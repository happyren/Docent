...
- **Legend editor** — declare your conventions as data: `dashed → async`, `red → hot-path`, `cylinder → datastore`. The exporter applies them. Authoring is point-and-click: the editor docks beside a live canvas — select any element, click the style chip that carries the meaning (color swatch, dash, shape…), and type only what it means. **The legend is drawn, not listed**: on the canvas each rule appears as the thing it describes with its meaning beside it — a blue ellipse next to *kind: datastore*, a dashed arrow next to *channel: async* — locked, above the diagram, never a component in any export, and never painted over by an agent.
...
- **Pleasing by construction.** What an agent draws is laid out by the layered pipeline done whole — median sweeps with a transpose pass until no crossing can be removed, Brandes–Köpf coordinates so flows run straight and a parent sits over its children, same‑kind shapes one size — with edges leaving at **spread ports**, nudged apart where they would run together, and — when something lies between their ends — routed around it **as one stroke**: the sides chosen by what the route costs (a return along a row goes over or under it as a U), jogs and hairpins removed, and every turn drawn by Docent as a true arc. A cycle is ranked by the order it was authored, so a `c → a` after `a → b → c` is the return, never a reason to draw the flow backwards. **Colour means something**: `define_kind` takes a `tone` (`danger`, `caution`, `positive`, `neutral`, `inactive`) or a `role` (`storage`, `compute`, `messaging`, `external`, `people`, `boundary`) and picks the conventional hue; without either, the fill farthest (CIELAB) from every kind in the legend, with a second channel (stroke style, then shape) past six kinds; conventional tags (`hot-path`, `deprecated`, `draft`…) colour their stroke.
- **Symbols — agents use the icon libraries by name.** A generated, checked-in catalog of every bundled library item (the Docent house glyphs, 249 AWS architecture icons and the software-architecture set, with aliases: `function` → Lambda, `queue` → SQS, `bucket` → S3); `find_symbol({query})` looks one up; `add_node({symbol:'aws/lambda', label})` places it as **one component** — the icon's own drawing, an invisible carrier on its bounds that arrows bind to and meaning lives on, and your label under it where the library puts its caption. `define_kind({kind, symbol})` puts the icon in the legend. Drag the icon and its arrows follow; tidy, routing and the score treat it like any component.
- **Drawings with taste.** A **house glyph library** — about twenty minimal, monochrome glyphs (user, service, database, queue, gateway, …) drawn in the canvas's own hand from a committed deterministic generator — answers brandless queries first: ask for "a queue" and the diagram gets the *idea* of a queue in pure ink; name Lambda and you get Lambda. And every symbol an **agent** places passes the house treatment: scribble fills become solid, stroke and roughness come down to the house's own, and every hue is kept — the colour is the brand, and the brand is meaning. Your own sidebar drags are never touched.
- **`tidy` is the formatter.** `tidy({frame})`, `{tier}`, `{selection}` or `{all:true}` re-lays out what it names — from **⌥⇧F**, the menu, or MCP — and is guaranteed to change nothing but the picture: its semantic changelog is empty, or the tidy is put back. It answers with the craft score before and after.
- **The craft score.** `validate()` returns a score out of 100 with its parts — crossings, bends, alignment, overlaps, edge lengths, squareness, colour distance — weighted as the research ranks them, per frame, with one line of advice each. The test suite watches a fixture's score, so the pipeline cannot regress quietly.
- **Laid out to be read.** A frame built in one batch is ranked by flow and ordered to avoid crossings; a **long flow turns** — more than five ranks fold into bands that run back beneath, so a pipeline stays a picture, not a ribbon; the gap between columns is **as wide as the words on the edge** between them, so a label never sits on the shapes at its ends; and **an edge never cuts through a component** — whatever lies between its ends, it is routed around on an orthogonal grid, fewest bends first, drawn in the house arrow's curvature, and the legend counts as in the way. `validate` reports an edge that passes through a component.
