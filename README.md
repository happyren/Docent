...
- **Pleasing by construction.** What an agent draws is laid out by the layered pipeline done whole — median sweeps with a transpose pass until no crossing can be removed, Brandes–Köpf coordinates so flows run straight and a parent sits over its children, same‑kind shapes one size — with edges leaving at **spread ports**, nudged apart where they would run together, and every turn **softened** into an arc. **Colour means something**: `define_kind` takes a `tone` (`danger`, `caution`, `positive`, `neutral`, `inactive`) or a `role` (`storage`, `compute`, `messaging`, `external`, `people`, `boundary`) and picks the conventional hue; without either, the fill farthest (CIELAB) from every kind in the legend, with a second channel (stroke style, then shape) past six kinds; conventional tags (`hot-path`, `deprecated`, `draft`…) colour their stroke.
