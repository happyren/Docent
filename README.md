...
- **Controls:** ▶ Present lives in the hamburger menu (as do all Docent actions — the canvas is full-bleed) · `→`/`Space` next · `←` prev · `Home` overview · click a linked component to dive into its detail diagram · `⌫` climbs back a tier · `Esc` exits. Selection-contextual actions (highlight, flow, drill) appear at the top of the same menu. Load any scene straight into the app with `?scene=<url>` (try `?scene=samples/demo.excalidraw`).
