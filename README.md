...
- **`tidy` is the formatter.** `tidy({frame})`, `{tier}`, `{selection}` or `{all:true}` re-lays out what it names — from **⌥⇧F**, the menu, or MCP — and is guaranteed to change nothing but the picture: its semantic changelog is empty, or the tidy is put back.
