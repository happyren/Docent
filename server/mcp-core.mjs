...
export const TOOLS = [
  ...
  },
  {
    ...
    name: "find_symbol",
    description:
      "Find a library symbol by name or meaning — 'lambda', 'queue', 'postgres', 'cdn' — in the bundled icon libraries (the Docent house glyphs, AWS architecture icons, software architecture). A brandless word answers the house glyph first (docent/queue, docent/database — minimal ink, drawn like the canvas); a vendor's own word answers the vendor (aws/lambda, aws/sqs). Answers ranked matches with id, name, library, category and size; pass the id as `symbol` to add_node.\nExample: find_symbol({query:'queue'}) → [{symbol:'docent/queue', …}, {symbol:'aws/sqs', …}]",
    inputSchema: {
      type: "object",
      properties: {
