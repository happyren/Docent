  ...
  switch (name) {
    ...
    case "draw":
      return [
        ...
        "4. edit({ops}) per frame — ONE batch holding the frame's components AND its edges, so the frame is laid out by flow with no crossings: add_node with label, kind, frame, intents (what it does, in the person's words), logic for rules; add_edge with a label or intents; use refs ($name) inside the batch. Keep labels short (a name, not a sentence); the sentence goes in intents.",
        "5. add_detail_layer for anything with an inner mechanism, then edit its components into that layer.",
        "6. validate(); fix warnings with update; if it reports arrow crossings, layout the frame or add a detail layer; propose before any large change.",
