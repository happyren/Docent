...
export const INSTRUCTIONS = [
  ...
  "You may also AUTHOR: add_node / add_edge / update / remove / add_frame / add_detail_layer / define_kind / use_genre / define_scenario / layout, or many at once with edit({ops}) — one validated, all-or-nothing batch, one undo step, answered with the semantic changelog; propose({ops}) is the same batch as a dry run. You author MEANING, never pixels: a component is a label + a `kind` (from the legend — define_kind adds one) + intents; Docent picks the shape, the style (the diagram's own), and the place. The craft: every component gets a kind and at least one intent; rules and conditions become `logic`; every frame gets a narrative; anything with an inner mechanism gets add_detail_layer rather than a crowded frame (≤12 per frame); reuse existing kinds before defining new ones. COLOUR MEANS SOMETHING: define_kind takes a `tone` (danger, caution, positive, neutral, inactive) or a `role` (storage, compute, messaging, external, people, boundary) and Docent picks a conventional, distinct look — with neither it picks the fill furthest from the legend's — and conventional tags (hot-path, deprecated, draft, critical…) colour themselves. SYMBOLS: BEFORE adding any component that is infrastructure or a product — a queue, a database, a function, a bus, a gateway, a browser, a device, anything with a brand name (AWS Lambda, S3, Kafka, EventBridge…) — call find_symbol; when the catalog holds a match, pass its id as `symbol` to add_node and the icon is placed as one component with the label under it. Never draw a product as a plain box when its symbol exists — a find_symbol miss costs one call, a plain box where an icon belongs costs the reader. The catalog answers in the house's taste: a brandless word (queue, database, gateway, user) answers a docent/ house glyph first — take it, that is the diagram's own hand — and a vendor's word answers the vendor's icon; name the vendor when you mean the vendor. The person's own library speaks too: items they have NAMED in their Excalidraw library answer as my/<name> and outrank everything — when a my/ symbol matches, it is the person's own word for the thing, so use it. Refs like `$orders` name things created earlier in the same batch. HOW DOCENT LAYS OUT, so you plan for it: a frame built in one batch is ranked by flow, left to right; a flow longer than five ranks FOLDS into bands that turn (right to left beneath, then left to right again), so a long sequence stays a picture, not a ribbon — plan a frame as a flow, never as a row; the gap between columns is sized to the widest edge label in it, so an edge label is a phrase (two to four words) and the sentence goes in the edge's intents; every edge is routed AROUND any component between its ends with turning points — an arrow never cuts through a shape, and never over the legend. ARROWS MUST NOT CROSS: build a frame in ONE batch (its components and its edges together) so Docent lays it out by flow; add a component with its edges in the same batch so it lands after its feeders; if validate() reports crossings or an edge passing through a component, layout({frame}) a frame you built, re-place the component, or move the tangle into a detail layer — never leave a crossing the diagram does not need. tidy({frame}|{tier}|{all:true}) formats any frame or the whole diagram and never changes meaning — use it after validate reports crossings, or before save_scene. Work in batches of a frame at a time, then validate() and fix what it lists, then save_scene(). validate() answers with a CRAFT SCORE out of 100 — its parts (crossings, bends, alignment, overlaps, edge lengths, squareness, colour), a score per frame, and one line of advice each — and a score under 70 means the picture is not ready: tidy the frame it names, or move the tangle a tier down with add_detail_layer, before you save. On a project bound to GitHub and sitting on its base branch (get_view().git.onBase), create_branch({name:'docent/<topic>'}) BEFORE the first edit. Where that base branch is protected (get_view().git.protected) this is not advice: canEdit comes back false with a `why`, and every write refuses with the same way forward until you have branched. While you edit the person sees an orange frame; keep batches short and tell them what you changed.",
].join("\n");
  ...
  switch (name) {
    ...
    case "draw":
      return [
        ...
        "2. Decide the kinds (service, datastore, queue, client, external…); get_scene_graph().legend first — reuse existing kinds; define_kind for each new one (shape optional); for named products and services, find_symbol and use `symbol` on add_node.",
...
export const TOOLS = [
  ...
  {
    // Reads the checked-in catalog (D81), never the canvas — a dispatcher
    // that holds the catalog answers this one itself (D82).
    name: "find_symbol",
    description:
      "Find a library symbol by name or meaning — 'lambda', 'queue', 'postgres', 'cdn' — in the bundled icon libraries (the Docent house glyphs, AWS architecture icons, software architecture) and, where a canvas is attached, the person's own named library items (my/<name> — the person's word for the thing, first among ties). A brandless word answers the house glyph first (docent/queue, docent/database — minimal ink, drawn like the canvas); a vendor's own word answers the vendor (aws/lambda, aws/sqs). Answers ranked matches with id, name, library, category and size; pass the id as `symbol` to add_node.\nExample: find_symbol({query:'queue'}) → [{symbol:'docent/queue', …}, {symbol:'aws/sqs', …}]",
    inputSchema: {
      type: "object",
      properties: {
        query: { type: "string", description: "A product, a service, or what the thing does" },
        library: { type: "string", description: "Only this library, e.g. 'aws-architecture-icons'" },
        limit: { type: "integer", minimum: 1, maximum: 20, description: "How many hits (default 8)" },
      },
      required: ["query"],
      additionalProperties: false,
    },
  ...
  {
    name: "define_proposal",
    description:
      "Record a proposal's case beside the legend (S24): a title, optionally what it is argued against ('base', 'saved', or 'project/path'), and the wins and costs — each one sentence. It exports with everything else and the outline says it; define_proposal({clear:true}) removes it. The workflow: create_branch, redraw the change, compare({against:'base'}) to see it, define_proposal to argue it, then present.\nExample: define_proposal({title:'Move billing to events', against:'base', wins:['Billing scales without the gateway'], costs:['One more hop before the receipt']})",
    inputSchema: {
      type: "object",
      properties: {
        title: { type: "string", description: "The proposal's one-line name" },
        against: { type: "string", description: "What the lens compares with — 'base', 'saved', or 'project/path'" },
  ...
  },
  {
    name: "compare",
    description:
      "Overlay the live canvas against a reference (S24, effects only — nothing is written): removed elements appear as dashed ghosts where they were, additions wear a green tint, changes amber; the answer carries the counts and both sides' craft scores. against:'base' is the synced base copy, 'saved' the stored copy of this scene, or {project, scene} any scene. compare({off:true}) or clear_effects puts the lens down.\nExample: compare({against:'base'})",
    inputSchema: {
      type: "object",
      properties: {
        against: {
          description: "'base', 'saved', or {project, scene}",
          anyOf: [
            { type: "string", enum: ["base", "saved"] },
            {
              type: "object",
        ...
        off: { type: "boolean", description: "True puts the lens down" },
      },
      additionalProperties: false,
    },
  },
